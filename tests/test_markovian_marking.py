...
def test_build_simple():
    ...
    # Check there is one entry per activity label (plus the empty trace)
    size_1_n_grams = [n_gram for n_gram in markovian_marking.markings if len(n_gram) == 1]
    assert len(size_1_n_grams) == len(reachability_graph.activity_to_edges) + 1
    ...
    assert markovian_marking.get_marking_state(["F"]) == {{25}}
